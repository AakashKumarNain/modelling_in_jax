...
class Embedding(ParamInitializer):
    ...
    embedding_logical_axes: tuple = dataclasses.field(default=("vocab_in", "embed_dim"), metadata=dict(static=True))

    @classmethod
    def param_specs(cls, vocab_size, embed_dim, dtype=jnp.float32, embedding_logical_axes=(None, None)):
        embedding = ParamSpec(
        ...
        return Embedding(vocab_size=vocab_size, embed_dim=embed_dim, embedding=embedding, embedding_logical_axes=embedding_logical_axes)

    @classmethod
    def init(
        cls, key, cfg,
        vocab_size, embed_dim,
        dtype=jnp.float32, embedding_logical_axes=(None, None)
    ):
        return cls._init_fn(
            key, cfg,
